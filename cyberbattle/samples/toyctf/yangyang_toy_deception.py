...
nodes = {
    "Website": m.NodeInfo(
        ...
        vulnerabilities=dict(
            ...
            connect_to_honeypot=m.VulnerabilityInfo(
                description="azurecredential.txt file in home directory",
                type=m.VulnerabilityType.LOCAL,
                outcome=m.LeakedCredentials(credentials=[m.CachedCredential(node="honeypot", port="HTTPS", credential="honeypot_credentials")]),
    ...
    "honeypot":m.NodeInfo(
        ...
        vulnerabilities=dict(
            ...
            searchhoneylink=m.VulnerabilityInfo(
               description="honeylink",
                type=m.VulnerabilityType.REMOTE,
                outcome=m.LeakedNodesId(["dummyMachine"]),
                reward_string="Web browser history revealed website URL of interest",
                cost=1.0,
            ),
        ),
    ...
    "client": m.NodeInfo(
        ...
        vulnerabilities=dict(
            ...
            SearchLocalHistory=m.VulnerabilityInfo(
                description="Search local history for list of accessed websites",
                type=m.VulnerabilityType.LOCAL,
                outcome=m.LeakedNodesId(["honeypot"]),
                reward_string="lcoal history revealed honeypot URL",
                cost=1.0,
            )
        ),
